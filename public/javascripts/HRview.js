$(document).ready(function () {
    ...
    $(document).on("change","#postSelection",function () {
        ...
        let send = {positionID: $("#postSelection").val(),acceptance: acceptance};
        $.post("/positionApplicant",send,function (data) {
            ...
            for(let i=0; i < data.length ; i++)
            {
                applicant = data[i];
                let row="<tr><td>\n" +
                    "<a href='#' role='button' value='"+applicant.id+"'>"+applicant.username+"</a>\n" +
                    "</td></tr>";
                $("#applicantTableBody").append(row);
            }
        });
    });
    $(document).on("click","#ApplicantNotification", function(){
        applicant();
        acceptance = false;
        $(document).on("click","a",function(){
            let application = {candidateID : this.id, positionID: $("#postSelection").val(),accepted : false};
    ...
    $(document).on("click","#approve",function(){
        ...
        function updatePrecedenceList() {
            ...
            for(let i in precedence)
            {
                ...
                $("select[name='precedence']").append(option);
            }
        }
